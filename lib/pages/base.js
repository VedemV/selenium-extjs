...
class BasePage {
	...
	/**
	 * Выполняет загрузку станицы, подключение `mochaUI`
	 * и ожидание элемента #locator
	 *
	 * @param {String} url
	 *
	 ...
	 */
	visit(url){
		// url 'data:,' не срабатывает для IE,
		// 'about:blank' принимают все браузеры
		if( typeof url === 'undefined' ) url = this.url;
			else if( url === null ) url = BasePage.EMPTYURL;
...
//			.then( _ => this.expected() );
	 ...
	 * в виде PromiseObject {x: значение, y: значение}.
	 *
	 * @return {!Object}
	 */
	get positionUI(){
...
/*
 * TODO: В статик отправить
 *
 * - FIXED: оригинальный WebElement.click,
 * - адрес пустой страницы выделить в константу
 * 
 */
};
