...
function condition( condition, doIf, doElse ){
	const chooseFn = bool => bool ? (doIf ? doIf() : undefined) : (doElse ? doElse() : undefined);
...
function parseBrowsers( config ){
	...
	//// or browsers: {chrome: <Capabilities>{}, firefox: <Capabilities>{}, etc.}
	// => config = [...]
//	let configNames = Array.isArray(config) ? config
//		: isString(config) ? config.split(',').map(item => String.prototype.trim.call(item))
//		: Object.keys(config);
